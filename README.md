...
| --- | --- |
| 应用包名 | `io.legado.read` |
| 当前版本 | `3.7.832`（以 `AppScope/app.json5` 为准） |
...
- 阅读页已接入 V2 分页布局：使用原生段落测量、增量分页、正文动作区域、正则样式、文本选区和 TTS 高亮叠加；阅读进度按字符偏移恢复并串行持久化，避免临时分页或旧页面覆盖当前位置；进入后台前会落定已接受的本地翻页，从 Home 或锁屏返回时重新发布当前正文表面，主题或背景变化时重挂原生正文表面，避免正文与页码不同步或保留旧样式；分页长按选词按当前页面和段落重新挂载原生选区，并用实际字形位置校准手柄和高亮，修复翻页模式下选区跨行、错位或菜单不显示；V2 不适用时仍保留现有分页路径。
...
| `versionName` | `3.7.832` |
| `versionCode` / `buildVersion` | `370832` |
| `minAPIVersion` | `12` |
| `targetAPIVersion` | `23` |
