...
| --- | --- |
| 应用包名 | `io.legado.read` |
| 当前版本 | `3.7.830`（以 `AppScope/app.json5` 为准） |
...
- 进程首次显示书架后会在后台静默检查连载书籍目录；检测到新章节时在封面显示“新”角标，进入书籍后清除，手动下拉刷新仍提供结果提示。
...
- 阅读页已接入 V2 分页布局：使用原生段落测量、增量分页、正文动作区域、正则样式、文本选区和 TTS 高亮叠加；阅读进度按字符偏移恢复并串行持久化，避免临时分页或旧页面覆盖当前位置；进入后台前会落定已接受的本地翻页，从 Home 或锁屏返回时重新发布当前正文表面，主题或背景变化时重挂原生正文表面，避免正文与页码不同步或保留旧样式；V2 不适用时仍保留现有分页路径。
...
- 连续阅读使用轻量正文分块并支持原生长按选词；段评面板可拖动调整高度，异常或超量的网页动作标记会被限制并从正文、复制和朗读文本中清理。

### 朗读与有声书

- 支持系统 TTS、自定义 HTTP TTS、语速、定时停止、章节切换、后台朗读和系统媒体控制；朗读状态可通过全局浮动控制会话在阅读页与书架之间继续操作。播放提示到达时会立即同步句子焦点，V2 高亮绘制在正文与正则气泡之上，减少朗读内容已经前进但高亮仍停留在上一句的问题。
...
- 书源、书架、章节进度、阅读设置、主题、书架展示配置和 TTS 配置按各自策略保存；眼动开关、横竖屏校准和停留时长纳入阅读设置与账号云同步，旧云端设置会保留本机新增字段并安排兼容回填。
...
| `versionName` | `3.7.830` |
| `versionCode` / `buildVersion` | `370830` |
| `minAPIVersion` | `12` |
| `targetAPIVersion` | `23` |
