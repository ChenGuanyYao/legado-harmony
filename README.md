...
- 公开仓库不提供真实书源、书源订阅地址或指向第三方内容站点的可导入配置；开发与测试应使用本地受控服务和合成数据。

## 功能概览

### 书架

- 管理普通书、漫画和有声书，支持继续阅读、批量管理、分组、排序、换源、缓存和删除。
- 支持平铺、紧凑和宽松布局，可配置封面、简介、标签、进度条、最近阅读大卡片和“书／漫／声”类型标识。
- 支持指定章节或全文缓存；全文缓存可转入后台并通过通知查看进度或取消。
- 支持最近阅读记录、桌面快捷项和桌面服务卡片。

### 搜索

- 基于已启用书源进行并发搜索，解析书名、作者、分类、封面、简介和详情地址。
- 搜索结果按批次增量加入列表，后台搜索期间仍可正常滑动；主页面签之间切换时保留当前结果，直到发起下一次搜索。
- 支持搜索进度、停止搜索、结果封面、标签、书架状态和搜索后返回书架提示等设置。
- 支持精准搜索，并可配置精准搜索时是否同时匹配作者。
- 支持书源分组筛选、单书源搜索和可调并发数，搜索设置与发现设置集中在搜索/发现配置页。
...
- 支持发现结果缓存；返回页面时优先恢复已加载内容，主动刷新时重新请求。

### 书源管理

- 支持从 URL、剪贴板或本地 JSON/文本文件导入书源。URL 导入依次尝试应用 HTTP、明文 HTTP TCP、ArkWeb 下载和系统下载服务，并输出可读错误。
- 导入时保存标准字段、规则对象、登录配置、运行参数和原始 JSON；导出时合并原始未知字段，减少复杂书源往返丢失。
- 支持选择模式下批量分组、启用/禁用、启用/禁用发现、锁定、导出和删除。
...
- 支持字号、行距、段落间距、页面边距、背景、深色模式、横向滑动和翻页方式设置。
...
- 接入系统 TTS 朗读能力，支持语速、定时停止、章节切换、后台朗读、系统媒体控制和全局播放胶囊。
...
- 有声书源使用独立远程音频播放器，支持播放/暂停、进度、倍速、上一章/下一章、目录跳转、定时停止和音色代码。
- 有声书播放可在返回书架或进入后台后继续，复用全局播放胶囊与系统 AVSession；从胶囊可重新打开播放面板、音色设置和章节目录。

### 数据与同步

- 支持应用数据备份/恢复、WebDAV 和账号云同步。
- 书源、书架、章节进度、阅读设置、主题、书架展示配置和 TTS 配置按各自同步策略保存。
- Android 备份导入会过滤不适合直接带入 HarmonyOS 启动路径的大块运行时缓存，同时保留可迁移阅读状态。

### 个性化

- 支持浅色/深色模式。
- 支持经典蓝、暖纸、林雾、水墨、霓虹、草莓奶霜、赤墨档案、梅影茶烟、玫瑰来信、月鹤星河、剑影青霜等主题包，统一 App 字体、语义色、阅读背景、正则气泡、圆角、玻璃材质与悬浮底栏图标。
- 支持按钮色、强调色的主题级默认值与用户微调。
- 正则字体支持跟随主题、文字强调、高亮色块，以及 QQ 柔和、QQ 实色、胶囊、纸笺、霓虹和十一套主题专属强调样式；局部与整段命中统一使用带主题镜像端饰的行内装饰框。
...
- URL 导入的多通道下载、BOM 清理、结构校验、可读错误和写入后完整性校验。
...
- 应用版本：`3.5.806`
- `versionCode` / `buildVersion`：`350806`
- `minAPIVersion`：`12`
- `targetAPIVersion`：`23`
...
4. 带 `loginUi` 的书源可在书源操作菜单中打开“登录与接口”面板；纯网页登录源按提示进入验证页完成登录和 Cookie 同步。
5. 返回首页后使用「搜索」查找书籍，或在「发现」中浏览已启用发现的书源。
6. 进入书籍详情，加入书架或直接阅读。
7. 在阅读页呼出菜单，进入阅读设置调整排版、朗读、预缓存和主题；有声书会进入独立播放面板。
...
- `[InteractionPostProcessor]`：段评、神评论、本章说和媒体动作后处理。
- `[RE]`：阅读引擎打开书籍、刷新目录和加载章节。
- `[TTS]` / `[HttpTtsReader]` / `[RemoteAudioPlayback]`：朗读音源、HTTP TTS、远程有声书、音频预取和播放状态。
...
- 分阶段 ArkWeb 可执行真实 JavaScript，但只开放受控桥接；脚本中的 `fetch`、`XMLHttpRequest`、`WebSocket`、任意 Java 导入、文件、进程、反射和系统组件不会因此自动可用。
- 非登录阶段的 `webView` / `webJs` URL 选项仍不会把普通请求自动切换成页面渲染抓取；登录动作中的 `java.webView` 与 `startBrowserAwait` 才有专用桥接。
- 需要浏览器指纹、长期动态渲染、强反爬、付费权限或私有登录流程的站点不保证支持。
- 聚合协议、段评和音频兼容尽量采用通用元数据与后处理，但后端私有字段、音色授权、会话 Token 和接口变更仍可能需要适配。
