...
| --- | --- |
| 应用包名 | `io.legado.read` |
| 当前版本 | `3.8.837`（以 `AppScope/app.json5` 为准） |
...
书源管理中的“调试”可针对单个书源运行指定阶段，也可运行全链路。搜索、发现、详情、目录和正文阶段支持分别输入关键词或 URL；执行明细会显示网络请求、响应摘要、规则类型、匹配数量、耗时和错误，便于定位“请求成功但字段为空”或“详情/目录地址未解析”等问题。
