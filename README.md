...
| --- | --- |
| 应用包名 | `io.legado.read` |
| 当前版本 | `3.7.826`（以 `AppScope/app.json5` 为准） |
...
- 阅读页已接入 V2 分页布局：使用原生段落测量、增量分页、正文动作区域、正则样式、文本选区和 TTS 高亮叠加；阅读进度按字符偏移恢复并串行持久化，避免临时分页或旧页面覆盖当前位置；V2 不适用时仍保留现有分页路径。
...
- 支持 `基础规则<js>处理脚本</js>后续规则` 的组合链，以及嵌套 `java.put/java.getString/java.timeFormatUTC` 等常用调用。
...
| `versionName` | `3.7.826` |
| `versionCode` / `buildVersion` | `370826` |
| `minAPIVersion` | `12` |
| `targetAPIVersion` | `23` |
